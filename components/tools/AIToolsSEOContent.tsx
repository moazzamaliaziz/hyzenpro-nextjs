...
export default async function AIToolsSEOContent() {
    ...
    try {
        ...
        // If a DB record exists and is explicitly disabled, don't render
        if (seoSection && !seoSection.enabled) {
            return null;
        }

        // If DB record has custom HTML, use it; otherwise use the hardcoded default
        const content = seoSection?.content as { html?: string } | null;
        if (content?.html && content.html.trim().length > 0) {
            html = content.html;
        }
    } catch {
        // On any DB error, gracefully fall back to the default SEO content
    }
