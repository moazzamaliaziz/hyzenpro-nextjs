...
import { ArrowLeft, Save, Loader2, Plus, X, Calendar, Clock, Eye, Trash2, RotateCcw, History, Undo2 } from 'lucide-react';
import Link from 'next/link';
import dynamic from 'next/dynamic';
import SEOFields from '@/components/admin/SEOFields';
import SlugInput from '@/components/admin/SlugInput';
import MediaLibraryLink from '@/components/admin/MediaLibraryLink';
import { normalizeCategory, normalizeCategories } from '@/lib/normalize-category';
...
export default function PostForm({ initialData, isEditing }: PostFormProps) {
    ...
    useEffect(() => {
        if (isEditing && initialData?.id) {
            ...
            fetch(`/api/posts/${initialData.id}/revisions`)
                .then(res => res.json())
    ...
    const handleSubmit = async (e: React.FormEvent, overrideStatus?: string) => {
        ...
        try {
            const resolvedAuthorName = authorId
                ? authors.find((a) => a.id === authorId)?.name
                : undefined;

            if (authorId && !resolvedAuthorName) {
                setSaving(false);
                setError('Selected author could not be resolved. Please re-select the author and try again.');
                return;
            }

            const body: any = {
                title, slug, excerpt, content, featuredImage,
                categories: normalizeCategories(categories), tags,
                status: finalStatus,
                postType,
                authorId: authorId || null,
                author: resolvedAuthorName ?? 'HyzenPro Team',
            ...
            if (isEditing && finalStatus !== 'trash') {
                fetch(`/api/posts/${initialData.id}/revisions`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        ...
                        content,
                        excerpt,
                        wordCount: calcReadingTime(content) * 200,
                    }),
                }).catch(() => {});
            }
    ...
    const handleRestoreRevision = async (revision: Revision) => {
        if (!confirm(`Restore this revision from ${new Date(revision.createdAt).toLocaleString()}? Current unsaved changes will be lost.`)) return;
        setRestoringId(revision.id);
        setTitle(revision.title);
        setContent(revision.content);
        if (revision.excerpt) setExcerpt(revision.excerpt);
        setRestoringId(null);
    };
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 space-y-4">
                        ...
                        <SlugInput
                            ...
                            sourceValue={title}
                            prefix="/blog/"
                            locked={Boolean(isEditing)}
                            lockedMessage="Editing a post will keep this URL stable. Create a redirect before changing published slugs."
                        />
                        <div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 space-y-4">
                        ...
                        <MediaLibraryLink helperText="Upload the image in the Media Library, then paste the generated HyzenPro URL here." />
                        {featuredImage && (
                    ...
                    {/* Tags */}
                    <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6">
                        <h3 className="font-heading text-base text-white mb-3">Tags</h3>
                        <div className="flex gap-2 mb-3">
                    ...
                    {isEditing && (
                        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6">
                            <h3 className="font-heading text-base text-white flex items-center gap-2 mb-3">
                            ...
                            {loadingRevisions ? (
                                ...
                                <p className="text-xs text-white/40">No saved revisions yet. Each time you save this post, a snapshot is stored here.</p>
                            ) : (
                                <div className="space-y-2 max-h-80 overflow-y-auto">
                                    {revisions.slice(0, 20).map((rev) => (
                                        <div key={rev.id} className="flex items-start justify-between gap-2 p-3 rounded-lg bg-white/[0.02] border border-white/[0.04] hover:border-white/10 transition-colors">
                                            <div className="min-w-0 flex-1">
                                                <p className="text-xs text-white/60 truncate">{rev.author}</p>
                                                <p className="text-[10px] text-white/30 mt-0.5">
                                                    {new Date(rev.createdAt).toLocaleString()}
                                                    {rev.wordCount && <span className="ml-2">{rev.wordCount} words</span>}
                                                </p>
                                            </div>
                                            <button
                                                type="button"
                                                ...
                                                className="shrink-0 p-1.5 rounded-md text-white/30 hover:text-accent hover:bg-accent/10 transition-colors disabled:opacity-50"
