...
export default function ComparePage() {
    ...
    if (!loading && selectedTools.length === 0) {
        return (
            <div className="min-h-screen pt-32 pb-24 px-4 bg-gray-50 flex flex-col items-center justify-center text-center">
                ...
                <Link href="/ai-tools-directory" className="px-6 py-3 bg-black text-white rounded-full font-bold uppercase tracking-widest text-sm hover:bg-gray-800 transition-colors">
                    Explore Directory
                </Link>
            </div>
        );
    ...
    return (
        <main id="main-content" tabIndex={-1} className="min-h-screen pt-28 pb-24 bg-white relative">
                            ...
                            {fullTools.map((tool) => (
                                <div key={tool.id} className="flex-1 min-w-[300px] max-w-[400px] bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all relative flex flex-col">
                                    ...
                                    <div className="p-6 border-b border-gray-100 bg-white">
                                         <div className="text-[10px] font-bold uppercase tracking-widest text-gray-600 mb-3">Pricing & Category</div>
                                        <div className="flex gap-2 mt-1">
                                            <span className="text-xs px-2.5 py-1 bg-gray-100 text-gray-600 rounded-full font-bold uppercase tracking-wider">{tool.pricingType}</span>
                                            <span className="text-xs px-2.5 py-1 bg-gray-100 text-black rounded-full font-bold uppercase tracking-wider">{tool.primaryCategory}</span>
                                        </div>
                                    </div>

                                    {/* Unified Features List */}
                                    <div className="p-6 border-b border-gray-100 bg-white">
                                         <div className="text-[10px] font-bold uppercase tracking-widest text-gray-600 mb-4">Core Capabilities</div>
                                    ...
                                    <div className="p-6 bg-gray-50 mt-auto flex flex-col gap-3">
                                        <a
                                            ...
                                            className="flex items-center justify-center gap-2 w-full py-3.5 bg-black text-white text-xs font-bold uppercase tracking-widest rounded-full hover:bg-gray-800 transition-all hover:-translate-y-0.5"
                                        >
                                            Visit Platform
                                        ...
                                        <Link
                                            ...
                                            className="flex items-center justify-center w-full py-3 bg-white border border-gray-200 text-black text-xs font-bold uppercase tracking-widest rounded-full hover:border-black hover:bg-gray-50 transition-colors"
                                        >
                                            Read Full Review
                                        </Link>
                                    </div>
